...
class UploadRepository(private val context: Context) {
    ...
    suspend fun uploadDocument(
        ...
        try {
            val userProfile = com.pravor.notessharing.core.firebase.FirestoreUserService().getUserProfile(uploaderId)
            if (userProfile != null) {
        ...
        val canonicalBranch = com.pravor.notessharing.core.util.LegacyAcademicCompatibilityResolver.resolveBranchId(branch)
        ...
        if (type == UploadType.Youtube) {
            ...
            if (isPlaylist) {
                val playlistId = youtubeUrl?.let { com.pravor.notessharing.domain.model.extractYoutubePlaylistId(it) } ?: ""
                ...
                val videoId = youtubeUrl?.let { com.pravor.notessharing.domain.model.extractYoutubeVideoId(it) } ?: ""
